...
@Component
public class DoorCloseConverter implements GridConverter<DoorCloseEvent, DoorClose> {

    @Override
    public DoorClose convert(DoorCloseEvent event) {
        DoorClose.DoorCloseBuilder<?, ?> builder = DoorClose.builder();
    ...
    @Override
    public List<DoorClose> convert(List<DoorCloseEvent> eventList) {
        List<DoorClose> doorCloses = new ArrayList<>();
        eventList.forEach(event -> doorCloses.add(convert(event)));
        return doorCloses;
    }
}
